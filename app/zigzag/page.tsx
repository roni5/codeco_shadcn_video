const DOTS_12 = Array.from({ length: 12 }).map((_, i) => ({ id: `dot-${i}` }));
...
export default function ZigzagSection() {
			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="grid md:grid-cols-2 gap-16 items-center mb-40 relative">
					...
					<div className="relative z-10">
						...
						<button
							type="button"
							className="bg-primary hover:bg-secondary text-white font-semibold px-8 py-3.5 rounded-lg transition-colors shadow-lg"
						>
							Try Demo
						</button>
					</div>
					...
					<div className="relative">
						...
						<div className="absolute -left-20 top-1/2 transform -translate-y-1/2 hidden lg:block z-20">
							<div className="grid grid-cols-3 gap-2">
								{DOTS_12.map(({ id }) => (
									<div key={id} className="w-2 h-2 bg-gray-300 rounded-full" />
								))}
							</div>
						</div>
	...
	/* Re-enable array-index key rule */
	/* biome-enable lint/suspicious/noArrayIndexKey */
}
