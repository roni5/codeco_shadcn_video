...
import { type HTMLAttributes, startTransition, useActionState, useEffect, useRef } from 'react'
import { useFormStatus } from 'react-dom'
import { useGoogleReCaptcha } from 'react-google-recaptcha-v3'
import { toast } from 'sonner'
import { type ContactFormState, createContact } from '../actions/contact/contactAction'
  ...
  useEffect(() => {
    if (formState.status === 'success') {
      sendGTMEvent({
        ...
        user_type: formState.form.email ? (formState.form.email.includes('@') ? 'business' : 'personal') : 'unknown',
    ...
    if (formState.status === 'field-errors') {
      sendGTMEvent({
        ...
        field_errors: Object.keys((formState.errors as Record<string, string>) || {}),
        page: 'contact_page'
      })
      ...
      <div className="relative w-full max-w-2xl">
        ...
        <div className="relative bg-background rounded-3xl overflow-hidden shadow-2xl">
          <div className="bg-linear-to-br from-primary to-secondary px-6 py-6 md:px-8 md:py-8 text-center">
            ...
            <p className="text-sm md:text-base text-primary-foreground/90">We&apos;d love to hear from you.</p>
          </div>
          ...
          <div className="px-6 py-6 md:px-8 md:py-8">
            <form ref={formRef} action={formAction} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="name" className="block mb-1.5 text-sm font-semibold text-foreground">
                    Your name
                  </label>
                  <Input
                    type="text"
                ...
                <div>
                  <label htmlFor="email" className="block mb-1.5 text-sm font-semibold text-foreground">
                    Email address
                  </label>
                  <Input
                    type="email"
                ...
                <div>
                  <label htmlFor="subject" className="block mb-1.5 text-sm font-semibold text-foreground">
                ...
                <div>
                  <label htmlFor="phone" className="block mb-1.5 text-sm font-semibold text-foreground">
              ...
              <div>
                <Label htmlFor="message" className="block mb-1.5 text-sm font-semibold text-foreground">
                  Message
                </Label>
                <Textarea
                  id="message"
              ...
              <div className="pt-2">
                <SubmitButton idleText="Send Message" submittingText="Sending..." />
              </div>
            </form>
          </div>
        </div>
